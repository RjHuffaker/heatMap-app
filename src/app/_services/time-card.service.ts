...
export class TimeCardService {
  ...
  getWeeklyTotal(week, user, ){
    ...
    week.forEach((day)=>{
      let timePunch = user.timeCard[day.format('YYMMDD')];
      if(timePunch){
        regular += timePunch[3];
      }
    });
  ...
  getPeriodTotal(month, start, end, user){
    let dates = month.slice(start, end);
    let startDate = dates[0];
    let endDate = dates[dates.length-1];
    let dateRange = startDate.format("YYMMDD")+"-"+endDate.format("DD");
    let week1 = this.getWeeklyTotal(dates.slice(0,7), user);
    let week2 = this.getWeeklyTotal(dates.slice(7,14), user);
    let week3 = this.getWeeklyTotal(dates.slice(14), user);
    
    let regular = week1.regular + week2.regular + week3.regular;
    let overtime = week1.overtime + week2.overtime + week3.overtime;
    let vacation = user.timeCard[dateRange] ? user.timeCard[dateRange][0] : 0;
  ...
  getYearlyTotals(date, user){
    ...
    for(var i = 1; i < 12; i++){
      ...
      let period1 = this.getPeriodTotal(month, 0, 15, user);
      let period2 = this.getPeriodTotal(month, 15, month.length+1, user);
    ...
    let firstPeriod = this.getPeriodTotal(firstMonth, 15, 31, user);
    let lastPeriod = this.getPeriodTotal(lastMonth, 0, 15, user);

    let nextPeriod = this.getPeriodTotal(lastMonth, 15, 31, user);
  ...
  checkDateRange(dateRange){
    let start = moment(dateRange.substring(0,6), 'YYMMDD');
    let end = moment(dateRange.substring(0,4)+dateRange.substring(7), 'YYMMDD');
    if(moment().isBefore(start) || moment().isAfter(end)){
      return "disabled";
    } else {
  ...
  getCurrentTotal(user, date?){
  	...
  	let total = this.getWeeklyTotal(week, user);
